...
its top-$k$ experts~\cite{shazeer2017outrageously}. After routing, the $T$
token--expert pairs are sorted by expert assignment, producing a contiguous
activation matrix $\mathbf{X} \in \mathbb{R}^{T \times D}$ where $D$ is the
...
$\sum_{e=1}^{E} n_e = T$. We partition rows of $\mathbf{X}$ into $E$
contiguous chunks $\mathbf{X}^{(e)} \in \mathbb{R}^{n_e \times D}$
according to the expert boundaries
$s_0 = 0,\; s_e = \sum_{i=1}^{e} n_i$.
...
\begin{definition}[M-GEMM: grouped matmul with variable $M$]
\label{def:mgemm}
...
\label{eq:mgemm}
\end{equation}
The $M$-dimension (rows of $\mathbf{A}$) varies across experts while
$K$ and $N$ are shared.
\end{definition}

\begin{definition}[K-GEMM: grouped matmul with variable $K$]
\label{def:kgemm}
...
\label{eq:kgemm}
\end{equation}
The reduction dimension $K$ (rows, i.e.\ tokens per expert) varies across
experts while the output shape $M \times N$ is fixed.
\end{definition}
...
FFN~\cite{nvidia2026megatronmoe}. The forward pass evaluates the
gated network $\mathbf{y} = (\mathrm{SiLU}(\mathbf{X}\,\mathbf{W}_{13}^{\top}) \odot \mathbf{W}_{13\text{-gate}}^{\top}) \,\mathbf{W}_{2}^{\top}$
via two M-GEMMs (GEMMs~1--2). The backward pass requires four grouped
...
$T$ is the total number of token--expert pairs, $D$ the hidden dimension,
...
per expert.
Existing systems either launch per-expert GEMMs sequentially, batch them via
multi-stream overlap, or fuse them into a single CUTLASS
kernel~\cite{nvidia2026megatronmoe}. While effective at hiding wave-tail
effects, these approaches do not fully exploit the structural properties of
M-GEMM and K-GEMM.
...
% TODO: Describe the proposed method for efficiently implementing
% the grouped GEMMs defined above.
